...
# Validate that data returned when a study guid is resolved is correct.
...
def get_guids_and_urls(BQ_client, storage_client, collection, args):
    aux = "`{}.{}.{}`".format(args.project, args.dataset, args.aux_table)
    sql = \
    """SELECT IDC_Webapp_CollectionID as collection, StudyInstanceUID, SeriesInstanceUID, SOPInstanceUID,  GCS_URL as url, CRDC_UUIDS.study as study_guid, CRDC_UUIDS.series as series_guid, CRDC_UUIDS.instance as instance_guid
    FROM {aux}
    ...
    for blob in storage_client.bucket(args.dst_bucket).list_blobs(prefix="instance_guids_and_urls_{}".format(collection)):
        ...
        rows = [{
            ...
            'study_guid': row[5],
            'series_guid': row[6],
            'instance_guid': row[7]} for row in less_raw]
...
def resolve_guid_hierarchical(guid, series_guids):
    response = requests.get('https://nci-crdc.datacommons.io/ga4gh/drs/v1/objects/{}'.format(guid))
    pass

def resolve_guid_expanded(guid, series_guids, guids_and_urls):
    query_string = {"expand": "true"}
    response = requests.get('https://nci-crdc.datacommons.io/ga4gh/drs/v1/objects/{}'.format(guid), \
                            params=query_string)
    if len(series_guids) == len(response.json()['contents']):
        for contentObject in response.json()['contents']:
            if not contentObject['id'] in series_guids:
                print('Resolved content series guid {} not in series guids'.format(contentObject['id']))
            instance_guids = set([row['instance_guid'] \
                            for row in guids_and_urls if contentObject['id'] == row['series_guid']])
            if len(instance_guids) == len(contentObject['contents']):
                for contentObject in contentObject['contents']:
                    if not contentObject['id'] in instance_guids:
                        print('Resolved instance content guid {} not in instance guids'.format(contentObject['id']))
                pass
...
def validate_collection(BQ_client, storage_client, collection, args):
    ...
    guids_and_urls = get_guids_and_urls(BQ_client, storage_client, collection, args)
    study_guids = set(row['study_guid'] for row in guids_and_urls)
    for guid in study_guids:
        if guid != "":
            series_guids = set([row['series_guid'] for row in guids_and_urls if guid == row['study_guid']])
            results = resolve_guid_expanded(guid, series_guids, guids_and_urls)
            results = resolve_guid_hierarchical(guid, series_guids)
